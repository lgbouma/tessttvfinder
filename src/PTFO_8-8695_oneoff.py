...
def scatter_plot_parameter_vs_epoch_manual(
    ...
    epoch, _ = get_epochs_given_midtimes_and_period(
        tmid[sel], lsfit_period, err_t_mid=None, t0_fixed=lsfit_t0
    )
    xvals = epoch
...
def make_transit_time_df(plname, manualtimecsv=None, tesstimecsv=None,
    ...
    if len(thispl) != 1:
        ...
        # init_period = 0.448463086010997
        # init_period = 0.448391
        init_period = 0.44840
...
def make_manually_curated_OminusC_plots(plname, datadir='../data/',
    ...
    for df, fname, init_period in list(
        ...
        if 'manual_plus_tess' in savdir or 'manual_O-C_vs_epoch' in savdir:
...
if __name__ == '__main__':
    ...
    # WASP-4b
    plname = 'PTFO_8-8695'
    manualtimecsv = '{:s}_manual_all.csv'.format(plname)
    occultationtimecsv = None # intentional
    savname = '{:s}_literature_and_TESS_times_O-C_vs_epoch_all.png'.format(plname)
    tesstimecsv = None
    ...
    # #occultationtimecsv = '{:s}_manual_occultations.csv'.format(plname)
    # savname = '{:s}_literature_and_TESS_times_O-C_vs_epoch.png'.format(plname)
    # tesstimecsv = '437248515_measured_TESS_times_7_transits.csv'
    # req_precision_minutes = 30 # get a junky one otherwise!
    # ylim = [-30,30]
    # correcterrorbars = True
